...
app.get('/commemorative_coins', (req, res) => {
    connection.query('SELECT * FROM coinctlg.coins;',
        (err, data) => {
...
//complete information on the coin
app.get('/coins/:id', (req, res) => {
    connection.query(`SELECT * FROM coinctlg.coins WHERE id = ${req.params.id};`,
        (err, data) => {
            if (err) return res.status(500);
            res.json(data);
        })
...
app.get('/coin/:id', (req, res) => {
    connection.query(`SELECT * FROM coinctlg.coins WHERE Title REGEXP '^${req.params.id}'
